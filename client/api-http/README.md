...
_Communication channel_ is used for encypted communication with contractor (another GEO Node). <br/>
It is an abstract layer that is used for securing all trust line-related operations between this pair of nodes. <br/>
It must be called **before** any other trust line-related operations with this contractor. <br/>
<br/>
This method performs **no communication** with the remote node so no traffic sniffing is possible. <br/>
The `secret key` that is generated by this method is used for traffic encryption between two nodes and must be **transferred securely** to the contractor node, so it is able to accept the channel on its side.
...
| ----                  | ----      | ----      | ----                      | ---- |
| `contractor_address`  | `Address` | +         | +                         | Specifies address of the remote GEO Node to which secure communication channel will be opened. The format: `<Type>-<Address>`. At the moment, only IPv4 addresses are supported in the following representation: `12-127.0.0.1:2002` (example).
| `crypto_key`          | `String`  |           |                           | In case of being called on the contractor's side, it's used to establish an outgoing connection to the original GEO Node that has initiated the operation.
| `contractor_id`       | `+Int`    |           |                           | In case of being called on the contractor's side, it specifies the channel id that should be used to establish connection to the original GEO Node that initiated the operation.
...
In case of success, the node reports additional data:
```json
{
  "data": {
...
This method provides a list of communication channels that are open on the current node.
...
This method creates an outgoing trust line from the current node to the contractor node that is specififed by the `contractor_id`.
...
| ----              | ----      | ---- |
| `contractor_id`   | `+Int`    | ID of the contractor to which the trust line should be open.
| `equivalent_id`   | `+Int`    | ID of the [equivalent](https://github.com/GEO-Protocol/specs-protocol/blob/master/trust_lines/trust_lines.md#trust-lines-equivalents) to which the trust line will be open.
...
| ----                  | ---- |
| `200 OK`              | Operation has been performed successfully.
| `401 Protocol Error`  | Node reports error. Please check node's log for the details.
...
In the event of new trust amount being equal to 0 - outgoing trust line would be closed. <br/>
In the event of incoming trust amount also being equal to 0 - current trust line would be archived. In this case no more operations would be allowed through this trust line.
...
| `200 OK`              | Operation has been performed successfully.
| `401 Protocol Error`  | Node reports error. Please check node's logs for the details.
| `406 Ins. keys`       | Operation can't be performed due to insufficient key count.
...
In the event of current trust line containing non-zero balance - trust line will not be closed, but incoming trust amount will be forced to 0. <br/> 
In the event of outgoing trust amount also being equal to 0 - current trust line would be archived. In this case no more operations would be allowed through this trust line.
...
| ---- | ---- |
| `200 OK` | Operation has been performed successfully.
| `401 Protocol Error` | Node reports error. Please, check node's logs for details.
| `406 Ins. keys` | Operation can't be performed due to insufficient key count.
...
To retrieve only a part of this list you should use the extended format of this method and specify `offset` and `count`.
...
This method returns a list of **all** [equivalents ids](https://github.com/GEO-Protocol/specs-protocol/blob/master/trust_lines/trust_lines.md#trust-lines-equivalents) that are present on the node. <br/>
...
This method returns a list of all contractors that are present on the node, and are operating in specififed `equivalent_id`.
...
| Parameter | Type | Required | Multiple <br/> Allowed | Description |
| ---- | ---- | ---- | ---- | ---- |
| `contractor_id` | `+Int` | + | | Specifies ID of the contractor trust line to/from which it should be returned.
...
| Parameter | Type | Required | Multiple <br/> Allowed | Description |
| ---- | ---- | ---- | ---- | ---- |
| `contractor_address` | `+Int` | + | + | Address(-es) of the contractor(s) to/from which trust lines are present.
...
This method initiates key pair exchange: current node generates and shares key pairs (up to the key's pool limit) with contractor node, and expects the same behavior from counterpart node.
...
| ---- | ---- | ---- |
| `equivalent_id` | `+Int` | ID of the [equivalent](https://github.com/GEO-Protocol/specs-protocol/blob/master/trust_lines/trust_lines.md#trust-lines-equivalents) that indicates the trust line.
| `contractor_id` | `+Int` | Specifies ID of the contractor trust line to/from which it should be updated.
...
This method combines routing algorithm and flow prediciton algorithm and tries to efficiently scan the network topology and to calculate max. payment flow from the issuer node to each one node that is listed in arguments. There is no need for direct TLs presence between issuer node and listed nodes: it will be enough if some paths exist that connect these nodes via some internal nodes. GEO Protocol supports paths that are up to 7 nodes long (icluding command issuer node and the destination node).
...
| Parameter | Type | Required | Multiple <br/> Allowed | Description |
| ---- | ---- | ---- | ---- | ---- |
| `contractor_address` | `+Int` | + | + | Address(-es) of the contractor(s) to/from which max flow should be calculated.
...
This method initialises payment operation from current node to the contractor. <br/>
Addresses received in arguments are used for communication with the remote node. Only one address is used at a time.
...
| Parameter | Type | Required | Multiple <br/> Allowed | Description |
| ---- | ---- | ---- | ---- | ---- |
| `contractor_address` | `+Int` | + | | Address of the contractor to/from which max flow should be calculated.
...
| `200 OK` | Operation has been performed successfully.
| `401 Protocol Error` | Node reports error. Please check node's logs for details.
| `409 No Consensus` | Consensus has not been reached (some nodes haven't approved the operation).
| `412 Isufficient Funds` | Insufficient payment possibilities.
| `413 Keys Error` | Payment is impossible due to keys lacking on one or several trust lines.
| `414 Keys Error` | Payment is impossible due to keys lacking on one or several middleware nodes.
...
This method returns all operations that are associated with a contractor (payment operations and trust line operations).
...
| `505` | Command transferring error. Node hasn't accepted the command.
